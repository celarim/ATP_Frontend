...
onMounted(async () => {
  ...
  const result_data = result.map((value) => value[1]["4. close"]);
  ...
  for (let i = 0; i < length; i++) {
    chartData.value.labels.push(result_label[i]);
    // chartData.value.datasets[0].data.push(parseFloat(result_data[i]));
  }
  chart.update();
...
<template>
  <div class="chart" style="position: relative; height: 25vh; width: 100vw">
