...
//입력한 주식 값 계산
const sum = computed(() => {
  return stockData.value.stocks.reduce((total, stock) => total + stock.quantity * stock.price, 0);
});
...
const chartData = computed(() => {
  ...
  const backgroundColor = labels.map((_, index) => usedColors.value[index] || generateUniqueColor());
  ...
  <div class="page-container">
    <div class="left-section">
      ...
      <div class="donut_name">
        <p v-if="!portfolioData.isEditing" @dblclick="enableEditing" class="editable-text" aria-placeholder="portfolio">
          {{ portfolioData.name }}
        </p>
        <input
          v-model="portfolioData.editName"
          v-else
          type="text"
          ...
          <div class="stock_name">
            <div class="stock_list">
              <input
                ...
                v-model="portStock.name"
                @focus="showList(index)"
                @input="showList(index)"
                @blur="hideList"
              />
              <img
                :class="['xmark', portStock.name === '' ? 'xmarkhide' : 'xmarkshow']"
                src="../images/x.svg"
        ...
        <div class="field-labels" v-for="(stock, index) in stockData.stocks" :key="index">
          ...
          <div class="stock_name">
            <div class="stock_list">
              ...
              />
              <img
                :class="['xmark', stock.name === '' ? 'xmarkhide' : 'xmarkshow']"
                src="../images/x.svg"
                @click="nameRemove(index)"
              />
              <ul v-if="isListVisible === index">
